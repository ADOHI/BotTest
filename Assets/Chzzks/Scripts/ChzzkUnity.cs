...
public class ChzzkUnity : MonoBehaviour
{
    ...
    public UnityEvent<string> onMessageWithNickName = new();
    ...
    private void Update()
    {
        ...
        if (messages.Count > 0)
        {
            ...
            {
                onMessageWithNickName?.Invoke(item);
            }
    ...
    private void DebugMessage(Profile profile, string str)
    {
        ...
        messages.Add($"{profile.nickname} : {str}");
        //onMessageWithNickName.Invoke($"{profile.nickname} : {str}");
    }
